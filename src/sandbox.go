...
func sand() {
	...
	// Endpoints acquired from Plaid Quickstart
...
func createLinkToken(c *gin.Context) {
	// Ensure products are configured; Plaid API returns INVALID_PRODUCT for empty products list
	products := convertProducts(strings.Split(PLAID_PRODUCTS, ","))
	if len(products) == 0 {
		c.JSON(http.StatusBadRequest, gin.H{"error": "no PLAID_PRODUCTS configured; set PLAID_PRODUCTS in your .env (e.g. 'transactions,auth')"})
		return
	}
...
func convertCountryCodes(countryCodeStrs []string) []plaid.CountryCode {
	...
	for _, countryCodeStr := range countryCodeStrs {
		countryCodeStr = strings.TrimSpace(countryCodeStr)
		if countryCodeStr == "" {
			continue
		}
		countryCodeStr = strings.ToUpper(countryCodeStr)
		countryCodes = append(countryCodes, plaid.CountryCode(countryCodeStr))
	}

	// Default to US if none provided to avoid Plaid returning an INVALID_FIELD
	if len(countryCodes) == 0 {
		countryCodes = append(countryCodes, plaid.CountryCode("US"))
	}
...
func linkTokenCreate(
	...
	// Set the user on the request separately (v40 constructor does not accept it)
...
func userTokenCreate() (string, error) {
	...
	if containsProduct(products, plaid.PRODUCTS_CRA_BASE_REPORT) ||
		...
		// The NewConsumerReportUserIdentity signature expects a nullable middle name
		// value before the addressData in v40; pass a nil/empty NullableString.
